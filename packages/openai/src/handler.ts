    ...
    try {
      ...
      for (let step = 0; step < maxSteps; step++) {
        ...
        for await (const chunk of stream) {
          ...
          if (delta.content) {
            emit(res, 'strand:text-delta', { delta: delta.content })
          }
          ...
          if (delta.tool_calls) {
            for (const tc of delta.tool_calls) {
              ...
              if (tc.id) {
                ...
                toolCallAccumulator.set(idx, { id: tc.id, name: tc.function?.name ?? '', argsJson: '' })
        ...
        if (finishReason === 'tool_calls' && toolCallAccumulator.size > 0) {
          ...
          const assistantToolCalls: OpenAI.ChatCompletionMessageToolCall[] = completedTools.map(tc => ({
            id: tc.id,
            type: 'function' as const,
            function: { name: tc.name, arguments: JSON.stringify(tc.input) },
          }))
          conversation.push({ role: 'assistant', tool_calls: assistantToolCalls, content: null })
